...
// MarkerUpdater: projects 3D sensor positions to 2D screen coordinates and reports them to parent
const MarkerUpdater = ({ sensors, setMarkerPositions }: { sensors: { id: string; position: [number, number, number] }[]; setMarkerPositions: any }) => {
  const { camera, size } = useThree();

  useFrame(() => {
    const positions = sensors.map(s => {
      const vec = new THREE.Vector3(...s.position);
      vec.project(camera);
      const x = (vec.x * 0.5 + 0.5) * size.width;
      const y = (-vec.y * 0.5 + 0.5) * size.height;
      const visible = vec.z < 1 && vec.z > -1;
      return { id: s.id, x, y, visible };
    });
...
export const Field3D = ({ onSensorClick, weather = 'sunny', timeOfDay = 'day' }: Field3DProps) => {
  ...
  const [markerPositions, setMarkerPositions] = useState<{ id: string; x: number; y: number; visible: boolean }[]>([]);
  ...
  return (
    <div className="relative w-full h-full">
      <Canvas 
        ...
        dpr={[1, 2]}
      >
        {/* visible background so we can tell if the canvas is rendering */}
        <color attach="background" args={["#f8fafc"]} />
        {/* axes helper for debugging orientation */}
        <axesHelper args={[5]} />
        {/* Enhanced Lighting */}
        ...
        {/* Marker updater runs inside the canvas and computes screen positions */}
        <MarkerUpdater sensors={sensors} setMarkerPositions={setMarkerPositions} />
      </Canvas>
      
      {/* HTML overlay for interactive sensor markers (projected from 3D) */}
      <div className="absolute inset-0 pointer-events-none">
        {markerPositions.map((m) => (
          <button
            key={m.id}
            onClick={() => handleSensorClick(m.id)}
            className="pointer-events-auto absolute -translate-x-1/2 -translate-y-1/2 flex items-center justify-center"
            style={{ left: `${m.x}px`, top: `${m.y}px` }}
            title={m.id}
          >
            <div className={`w-4 h-4 rounded-full shadow-lg border-2 ${m.visible ? 'bg-green-500 border-white' : 'bg-gray-300 border-white/80'}`} />
          </button>
        ))}
      </div>
